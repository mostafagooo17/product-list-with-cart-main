...
function getData(filejson) {
  ...
  xhr.onload = function () {
    if (xhr.status >= 200 && xhr.status < 300) {
      ...
      if (!data) data = JSON.parse(xhr.responseText); // Fallback for some browsers
      ...
      console.error("Error loading data.json: ", xhr.status, xhr.statusText);
    }
  };

  xhr.onerror = function () {
    console.error("Request failed. Please check your local server setup.");
  };
